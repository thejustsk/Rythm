...
- [x] M7 — Insights & analytics: summary cards (planned/completed/completion %/streak), plain-language digest, time-per-label donut, planned-vs-done bars, busiest-hours & day-of-week charts, 16-week activity heatmap, per-label completion bars, This week/Month/Year/All-time periods; custom SVG charts (no extra deps); status colour dots (todo=grey, doing=blue pulse, cancelled=red; done = none, struck+dimmed); week table scrollbar-gutter fix; verified by smoke test (112 checks)
