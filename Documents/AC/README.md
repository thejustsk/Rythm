...
- [x] M10.3 — Milestones & celebration: **4 KPI cards in a fixed non-scrolling band**; **milestone path grows ONE stone at a time** (starts with Level 1 = 100; finishing it adds Level 2 = 250, etc; level names auto-increment, costs fixed); **the NEXT stone appears the moment the previous is REACHED (balance ≥ cost) — not claimed**; stack shows descending with Level 1 at the bottom; **only the reward NOTE is editable** (single-field ✎ dialog + popup); redeem button has no bracket stat; buttons never cascade on small screens; 7-day chart width matches the KPI card exactly; redeemable multiple times with gold redeem-count + gold ✎; **Total earned / Today's earning subtract refunds**; sidebar coin chip updates instantly on bonuses; **"How did it go?" closes IMMEDIATELY** on selection (no lingering animation); **professional gold-coin intro**: big designed coin drops with a bounce, gold-dust burst + expanding ring, KPI cards flip in after — all transparent (no white screen); **new crafted gold coin** (star emblem, milled rim, gloss — `assets/coin-gold.png`) now actually visible in toolbar/tab/sidebar/KPI/intro; designed Rhythm coin (blue/gold, tsk) visible in header/tab/sidebar/KPI + workspace asset `coin-design.svg`/`.png`; **M10 gamification complete** 🎉
